...
<head>
    ...
    function change_dep() {
        ...
        document.location.href = "rpp_main.php?departemen="+departemen+"&semester="+semester+"&pelajaran="+pelajaran;
    }
    ...
    function change() {
        ...
        document.location.href = "rpp_main.php?departemen="+departemen+"&semester="+semester+"&tingkat="+tingkat+"&pelajaran="+pelajaran;
    }
    ...
    function show() {
        ...
        document.getElementById('footer_frame').src = "rpp_footer.php?departemen="+departemen+"&tingkat="+tingkat+"&semester="+semester+"&pelajaran="+pelajaran;
    }
...
<body class="bg-green-950 text-slate-800 min-h-screen p-4 md:p-6 select-none overflow-x-hidden">
    ...
    <div class="w-full h-[calc(100vh-3rem)] bg-slate-50 rounded-[2.5rem] md:rounded-[3rem] shadow-2xl border border-green-800/30 p-6 md:p-10 flex flex-col">
        ...
        <div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm mb-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div class="flex flex-wrap items-center gap-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative">
                        <select name="departemen" id="departemen" onChange="change_dep()" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-32 pl-3 pr-8 py-2.5 shadow-sm transition-colors cursor-pointer" onKeyPress="return focusNext('tingkat', event)">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative">
                        <select name="tingkat" id="tingkat" onChange="change()" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-32 pl-3 pr-8 py-2.5 shadow-sm transition-colors cursor-pointer" onKeyPress="return focusNext('semester', event)">
                            <? 
                            ...
                            while($row_t = mysqli_fetch_array($result_t)) {
                                ...
                                <option value="<?=urlencode($row_t['replid'])?>" <?=IntIsSelected($row_t['replid'], $tingkat) ?>><?=$row_t['tingkat']?></option>
                            <? } ?>
                        </select>
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2.5 text-slate-500">
                            <i class="fa-solid fa-chevron-down text-[10px]"></i>
                ...
                <!-- Semester -->
                <div class="flex items-center gap-3">
                    <label for="semester" class="text-sm font-bold text-slate-700 uppercase tracking-wider text-xs">Semester</label>
                    <div class="relative">
                        <select name="semester" id="semester" onChange="change()" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-48 pl-3 pr-8 py-2.5 shadow-sm transition-colors cursor-pointer" onKeyPress="return focusNext('pelajaran', event)">
                            <?
                            ...
                            while ($row_s = @mysqli_fetch_array($result_s)) {
                                ...
                                <option value="<?=urlencode($row_s['replid'])?>" <?=IntIsSelected($row_s['replid'], $semester)?> ><?=$row_s['semester']." ".$ada?></option>                 
                            <? } ?>
                        </select>
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2.5 text-slate-500">
                            <i class="fa-solid fa-chevron-down text-[10px]"></i>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative">
                        <select name="pelajaran" id="pelajaran" onChange="change()" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-56 pl-3 pr-8 py-2.5 shadow-sm transition-colors cursor-pointer" onKeyPress="return focusNext('tabel', event)">
                            <?
                            ...
                            while ($row_p = @mysqli_fetch_array($result_p)) {
                                ...
                                <option value="<?=urlencode($row_p['replid'])?>" <?=IntIsSelected($row_p['replid'], $pelajaran)?> ><?=$row_p['nama']?></option>
                            <? } ?>
                        </select>
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2.5 text-slate-500">
                            <i class="fa-solid fa-chevron-down text-[10px]"></i>
            ...
            <div class="flex items-center gap-2.5">
                <button id="tabel" onClick="show()" class="flex items-center gap-2 bg-emerald-900 hover:bg-emerald-800 text-white font-bold text-xs py-2.5 px-6 rounded-xl shadow-md transition-all duration-200 active:scale-95">
                    <i class="fa-solid fa-eye"></i> Tampilkan RPP
                </button>
            </div>
        </div>
        ...
        <div class="flex-1 bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden relative">
            <iframe name="footer" id="footer_frame" src="blank_rpp.php" class="w-full h-full border-none"></iframe>
        </div>
