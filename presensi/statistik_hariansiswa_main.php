...
<head>
    ...
    function tampil() {
        ...
        document.getElementById('footer_frame').src = "statistik_hariansiswa_footer.php?bln1="+bln1+"&th1="+th1+"&bln2="+bln2+"&th2="+th2+"&semester="+semester+"&kelas="+kelas;
    }
    ...
    function change() {
        ...
        document.location.href = "statistik_hariansiswa_main.php?bln1="+bln1+"&th1="+th1+"&bln2="+bln2+"&th2="+th2+"&departemen="+departemen+"&semester="+semester+"&tahunajaran="+tahunajaran+"&tingkat="+tingkat+"&kelas="+kelas;	
    }
...
<body class="bg-green-950 text-slate-800 min-h-screen p-4 md:p-6 select-none overflow-x-hidden" onload="document.getElementById('departemen').focus()">
    ...
    <div class="w-full h-[calc(100vh-3rem)] bg-slate-50 rounded-[2.5rem] md:rounded-[3rem] shadow-2xl border border-green-800/30 p-6 md:p-10 flex flex-col">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div class="flex items-center gap-4 bg-white p-4 rounded-3xl border border-green-100 shadow-sm flex-1">
                ...
                <div>
                    ...
                    <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">STATISTIK KEHADIRAN HARIAN SISWA</h1>
                </div>
            </div>
            ...
            <div class="bg-slate-100 px-5 py-3 rounded-2xl border border-slate-200 self-start md:self-center text-xs flex items-center gap-2">
                ...
                <span class="text-slate-600 font-medium">Statistik Harian Siswa</span>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm mb-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <div class="relative">
                        <select name="kelas" id="kelas" onChange="change()" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 block w-full pl-3 pr-8 py-2.5 shadow-sm cursor-pointer">
                            <?
            ...
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mt-6">
                <div class="flex flex-wrap items-center gap-6">
                    <div class="flex items-center gap-3">
                        <label class="text-sm font-bold text-slate-700 uppercase tracking-wider text-xs">Bulan Mulai</label>
                        <div class="flex gap-1.5">
                            <div class="relative">
                                <select name="bln1" id="bln1" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 block w-24 pl-3 pr-6 py-2.5 shadow-sm cursor-pointer">
                                    <? for ($i=1;$i<=12;$i++) { ?>
                            ...
                            <div class="relative">
                                <select name="th1" id="th1" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 block w-24 pl-3 pr-6 py-2.5 shadow-sm cursor-pointer">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex gap-1.5">
                            <div class="relative">
                                <select name="bln2" id="bln2" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 block w-24 pl-3 pr-6 py-2.5 shadow-sm cursor-pointer">
                                    <? for ($i=1;$i<=12;$i++) { ?>
                            ...
                            <div class="relative">
                                <select name="th2" id="th2" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 block w-24 pl-3 pr-6 py-2.5 shadow-sm cursor-pointer">
                                    <? for ($i = $tahun1; $i <= $tahun2; $i++) { ?>
        ...
        <div class="flex-1 bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden relative">
            <iframe name="footer" id="footer_frame" src="blank_statistik_kehadiran_siswa.php" class="w-full h-full border-none"></iframe>
        </div>
