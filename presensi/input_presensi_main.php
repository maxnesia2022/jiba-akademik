...
<head>
    ...
    function change_departemen(){
        ...
        document.location.href="input_presensi_main.php?departemen="+departemen;
    }
    ...
    function change(){
        ...
        document.location.href="input_presensi_main.php?departemen="+departemen+"&tingkat="+tingkat+"&tahunajaran="+tahunajaran+"&kelas="+kelas+"&semester="+semester;
    }
    ...
    function change_tingkat() {
        ...
        var semester = document.getElementById("semester").value;
        var tingkat = document.getElementById("tingkat").value;
        document.location.href = "input_presensi_main.php?departemen="+departemen+"&tahunajaran="+tahunajaran+"&semester="+semester+"&tingkat="+tingkat;
    }
    ...
    function tampil(){
        ...
        if (semester==""){
            ...
            document.getElementById('footer_frame').src = "input_presensi_footer.php?semester="+semester+"&kelas="+kelas;
        }
    }	
...
<body class="bg-green-950 text-slate-800 min-h-screen p-4 md:p-6 select-none overflow-x-hidden" onload="document.getElementById('departemen').focus()">
    ...
    <div class="w-full h-[calc(100vh-3rem)] bg-slate-50 rounded-[2.5rem] md:rounded-[3rem] shadow-2xl border border-green-800/30 p-6 md:p-10 flex flex-col">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div class="flex items-center gap-4 bg-white p-4 rounded-3xl border border-green-100 shadow-sm flex-1">
                ...
                <div>
                    ...
                    <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">INPUT PRESENSI HARIAN</h1>
                </div>
            </div>
            ...
            <div class="bg-slate-100 px-5 py-3 rounded-2xl border border-slate-200 self-start md:self-center text-xs flex items-center gap-2">
                ...
                <span class="text-slate-600 font-medium">Input Presensi Harian</span>
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm mb-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div class="flex flex-wrap items-center gap-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <?  
                    $sql = "SELECT replid,tahunajaran FROM tahunajaran WHERE departemen='$departemen' AND aktif=1 ORDER BY replid DESC LIMIT 1";
                    $result = QueryDb($sql);
                    $row = @mysqli_fetch_array($result);	
                    ?>
                ...
                <!-- Semester -->
                <div class="flex items-center gap-3">
                    <label for="sem" class="text-sm font-bold text-slate-700 uppercase tracking-wider text-xs">Semester</label>
                    <?	
                    $sql = "SELECT replid,semester FROM semester where departemen='$departemen' AND aktif = 1 ORDER BY replid DESC LIMIT 1";
                    $result = QueryDb($sql);
                    $row = @mysqli_fetch_array($result);			
                    ?>
                    <input type="text" name="sem" id="sem" readonly class="bg-slate-100 border border-slate-200 text-slate-500 text-xs font-bold rounded-xl px-4 py-2.5 w-44 shadow-sm" value="<?=$row['semester']?>" />
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative">
                        <select name="tingkat" id="tingkat" onChange="change()" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-32 pl-3 pr-8 py-2.5 shadow-sm transition-colors cursor-pointer">
                            <?
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative">
                        <select name="kelas" id="kelas" onChange="change()" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-44 pl-3 pr-8 py-2.5 shadow-sm transition-colors cursor-pointer">
                            <?
        ...
        <div class="flex-1 bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden relative">
            <iframe name="footer" id="footer_frame" src="blank_presensi.php?tipe='harian'" class="w-full h-full border-none"></iframe>
        </div>
