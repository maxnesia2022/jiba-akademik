...
<head>
    ...
    <title>Pendataan Semester</title>
    <!-- Tailwind CSS CDN -->
    ...
    function tambah() {
        ...
        window.open('semester_add.php?departemen='+departemen, 'TambahSemester','width=500,height=300,resizable=1,scrollbars=1');
    }
    ...
    function setaktif(replid, aktif) {
        var departemen = document.getElementById('departemen').value;
        var msg = (aktif == 1) ? "Non-aktifkan semester ini?" : "Aktifkan semester ini?";
    ...
    function edit(replid) {
        window.open('semester_edit.php?replid='+replid, 'UbahSemester','width=500,height=300,resizable=1,scrollbars=1');
    }
    ...
    function cetak() {
        var departemen = document.getElementById('departemen').value;
        window.open('semester_cetak.php?departemen='+departemen, 'CetakSemester','width=790,height=650,resizable=1,scrollbars=1');
    }
    </script>
    <style>
...
<body class="bg-emerald-950 text-slate-800 min-h-screen p-4 md:p-6 overflow-x-hidden" onload="document.getElementById('departemen').focus()">
    ...
    <div class="w-full min-h-[calc(100vh-3rem)] bg-slate-50 rounded-[2.5rem] md:rounded-[3rem] shadow-2xl border border-emerald-800/30 p-6 md:p-10">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div class="flex items-center gap-4 bg-white p-4 rounded-3xl border border-green-100 shadow-sm flex-1">
                ...
                <div>
                    ...
                    <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">PENDATAAN SEMESTER</h1>
                </div>
            </div>
            ...
            <div class="bg-slate-100 px-5 py-3 rounded-2xl border border-slate-200 self-start md:self-center text-xs flex items-center gap-2">
                ...
                <span class="text-slate-600 font-medium">Semester</span>
            </div>
        </div>
                ...
                <tbody class="divide-y divide-slate-100 text-sm font-medium text-slate-800">
                    ...
                    while ($row = @mysqli_fetch_array($result)) { ?>
                        <tr class="hover:bg-slate-50/50 transition-colors duration-150">
                            ...
                            <td class="py-3 px-6 text-emerald-850 font-bold"><?=$row['semester'] ?></td>
                            ...
                            <td class="py-3 px-6 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button onClick="JavaScript:edit(<?=$row['replid'] ?>)" class="p-2 text-sky-600 hover:bg-sky-50 rounded-xl transition-colors" title="Ubah Semester">
                                        <i class="fa-solid fa-pen-to-square text-base"></i>
                                    </button>
                                    <button onClick="JavaScript:hapus(<?=$row['replid'] ?>)" class="p-2 text-rose-600 hover:bg-rose-50 rounded-xl transition-colors" title="Hapus Semester">
        ...
        <?php } else { ?>
            ...
            <div class="bg-white rounded-3xl border border-slate-100 shadow-md p-12 text-center max-w-xl mx-auto mt-12">
                ...
                <p class="text-xs text-slate-500 mb-6">Silakan periksa filter departemen Anda atau buat data semester baru.</p>
                <?php if (SI_USER_LEVEL() != $SI_USER_STAFF) { ?>
