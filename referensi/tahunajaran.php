...
if (isset($_REQUEST['action'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($_REQUEST['action'] == "add") {
            ...
            $sql = "UPDATE tahunajaran SET tahunajaran = '".CQ($_REQUEST['tahunajaran'])."', tglmulai = '$tglmulai', tglakhir = '$tglakhir', keterangan = '".CQ($_REQUEST['keterangan'])."' WHERE replid = '$_REQUEST[replid]'";
            $result = QueryDb($sql);            
        }
        
        if ($result) {
            echo "<script>window.location.href='tahunajaran.php?departemen=".$_REQUEST['departemen']."';</script>";
            exit;
        }
    }
}
...
<head>
    ...
    function setaktif(replid, aktif) {
        var departemen=document.getElementById("departemen").value;
        var msg = (aktif == 1) ? "Non-aktifkan Tahun Ajaran ini?" : "Aktifkan Tahun Ajaran ini?";
...
<body class="bg-green-950 text-slate-800 min-h-screen p-4 md:p-6 overflow-x-hidden" onload="document.getElementById('departemen').focus()">
