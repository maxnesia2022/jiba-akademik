    ...
    <div class="w-full min-h-[calc(100vh-3rem)] bg-slate-50 rounded-[2.5rem] md:rounded-[3rem] shadow-2xl border border-green-800/30 p-6 md:p-10">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div class="flex items-center gap-4 bg-white p-4 rounded-3xl border border-green-100 shadow-sm flex-1">
                ...
                <div>
                    ...
                    <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">PENDATAAN DEPARTEMEN</h1>
                </div>
            </div>
            ...
            <div class="bg-slate-100 px-5 py-3 rounded-2xl border border-slate-200 self-start md:self-center text-xs flex items-center gap-2">
                ...
                <span class="text-slate-600 font-medium">Departemen</span>
            </div>
        </div>
        ...
        <?	$sql = "SELECT d.replid,d.departemen,d.nipkepsek,p.nama,d.keterangan,d.aktif 
            ...
            <div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm mb-6 flex items-center justify-between">
                <span class="text-sm font-semibold text-slate-500">Daftar Departemen Sekolah</span>
                ...
                <table class="w-full text-left border-collapse" id="table">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100 text-slate-700 uppercase text-xs font-bold tracking-wider">
                            ...
                            <th class="py-4 px-6">Kepala Sekolah</th>
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm font-medium text-slate-800">
                        <? 	$cnt = 0;
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors duration-150">
                                ...
                                <td class="py-3 px-6 text-emerald-850 font-bold"><?=$row[1] ?></td>
                                <td class="py-3 px-6">
                                    <div class="flex flex-col">
                                        <span class="text-slate-900 font-bold"><?=$row[3] ?></span>
                                        <span class="text-xs text-slate-400 font-semibold">NIP. <?=$row[2] ?></span>
                                    </div>
                                </td>
                                <td class="py-3 px-6 text-slate-500"><?=$row[4] ?></td>
                                <td class="py-3 px-6 text-center">
                                    <? if (SI_USER_LEVEL() == $SI_USER_STAFF) { ?>
                                        ...
                                        <? } ?>
                                    <? } else { ?>
                                        <a href="JavaScript:setaktif(<?=$row[0] ?>, <?=$row[5] ?>)" class="inline-block transition-transform active:scale-95">
                                            <? if ($row[5] == 1) { ?>
                                ...
                                <td class="py-3 px-6 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <button onClick="JavaScript:edit(<?=$row[0] ?>)" class="p-2 text-sky-600 hover:bg-sky-50 rounded-xl transition-colors" title="Ubah Departemen">
                                            <i class="fa-solid fa-pen-to-square text-base"></i>
                                        </button>
                                        <button onClick="JavaScript:hapus(<?=$row[0] ?>)" class="p-2 text-rose-600 hover:bg-rose-50 rounded-xl transition-colors" title="Hapus Departemen">
        ...
        <? } else { ?>
            ...
            <div class="bg-white rounded-3xl border border-slate-100 shadow-md p-12 text-center max-w-xl mx-auto mt-12">
                ...
                <p class="text-xs text-slate-500 mb-6">Silakan buat data departemen sekolah baru untuk memulai manajemen modul.</p>
                <? if (SI_USER_LEVEL() != $SI_USER_STAFF) { ?>
                    <button onClick="JavaScript:tambah()" class="inline-flex items-center gap-2 bg-emerald-950 hover:bg-emerald-900 text-white font-bold text-xs py-3 px-6 rounded-2xl shadow-md transition-all duration-200 active:scale-95">
                        <i class="fa-solid fa-plus"></i> Tambah Departemen Baru
                    </button>
                <? } ?>
            </div>
        <? } ?> 
