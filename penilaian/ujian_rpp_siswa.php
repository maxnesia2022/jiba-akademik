...
if ($kelas == "" && $departemen != "" && $tingkat != "") {
    $query_k = "SELECT k.replid FROM kelas k, tahunajaran a WHERE k.idtingkat = '$tingkat' AND k.aktif = 1 AND k.idtahunajaran = a.replid AND a.departemen = '$departemen' AND a.aktif = 1 ORDER BY k.kelas LIMIT 1";
    $result_k = QueryDb($query_k);
    if ($row_k = @mysqli_fetch_array($result_k)) {
        $kelas = $row_k['replid'];
    }
}
...
<head>
    ...
    <title>Rata-rata RPP Setiap Siswa</title>
    <!-- Tailwind CSS CDN -->
    ...
    function change_sel(){
        ...
        document.location.href="ujian_rpp_siswa.php?departemen="+departemen;
    }
    ...
    function change_sel2() {
        ...
        document.location.href="ujian_rpp_siswa.php?tingkat="+tingkat+"&departemen="+departemen+"&pelajaran="+pelajaran;
    }
    ...
    function show(){
        ...
        if(dep.length == 0) {
            ...
            document.getElementById('footer_frame').src="ujian_rpp_siswa_footer.php?departemen="+dep+"&pelajaran="+pelajaran+"&semester="+semester+"&kelas="+kelas;
        }
    }
...
<body class="bg-green-950 text-slate-800 min-h-screen p-4 md:p-6 select-none overflow-x-hidden" onload="document.getElementById('departemen').focus()">
    ...
    <div class="w-full h-[calc(100vh-3rem)] bg-slate-50 rounded-[2.5rem] md:rounded-[3rem] shadow-2xl border border-green-800/30 p-6 md:p-10 flex flex-col">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div class="flex items-center gap-4 bg-white p-4 rounded-3xl border border-green-100 shadow-sm flex-1">
                ...
                <div>
                    ...
                    <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">RATA-RATA RPP SETIAP SISWA</h1>
                </div>
            </div>
            ...
            <div class="bg-slate-100 px-5 py-3 rounded-2xl border border-slate-200 self-start md:self-center text-xs flex items-center gap-2">
                ...
                <span class="text-slate-600 font-medium">Rata-rata RPP Siswa</span>
            </div>
        </div>
            ...
            <div class="flex flex-wrap items-center gap-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex gap-2">
                        ...
                        <div class="relative">
                            <select name="kelas" id="kelas" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-32 pl-3 pr-8 py-2.5 shadow-sm transition-colors cursor-pointer">
                                <?
                                $query_k = "SELECT k.replid, k.kelas FROM kelas k, tahunajaran a WHERE k.idtingkat = '$tingkat' AND k.aktif = 1 AND k.idtahunajaran = a.replid AND a.departemen = '$departemen' AND a.aktif = 1 ORDER BY k.kelas";
                                $result_k = QueryDb($query_k);
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative">
                        <select name="pelajaran" id="pelajaran" onChange="change_sel2()" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-44 pl-3 pr-8 py-2.5 shadow-sm transition-colors cursor-pointer">
                            <? 
        ...
        <div class="flex-1 bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden relative">
            <iframe name="footer" id="footer_frame" src="blank_rppsiswa.php" class="w-full h-full border-none"></iframe>
        </div>
