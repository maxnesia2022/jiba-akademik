...
<head>
    ...
    function change_dep() {
        ...
        document.location.href = "statistik_mutasi_siswa.php?departemen="+departemen;
    }
    ...
    function change() {
        ...
        if (tahunakhir < tahunawal){
            alert ('Tahun akhir tidak boleh kurang dari tahun awal!');
            return false;
        }	
        document.location.href = "statistik_mutasi_siswa.php?departemen="+departemen+"&tahunawal="+tahunawal+"&tahunakhir="+tahunakhir;
    }
    ...
    function tampil() {
        ...
        document.getElementById('footer_frame').src = "content_mutasi.php?departemen="+departemen+"&tahunawal="+tahunawal+"&tahunakhir="+tahunakhir;
    }
...
<body class="bg-green-950 text-slate-800 min-h-screen p-4 md:p-6 select-none overflow-x-hidden" onload="document.getElementById('departemen').focus()">
    ...
    <div class="w-full h-[calc(100vh-3rem)] bg-slate-50 rounded-[2.5rem] md:rounded-[3rem] shadow-2xl border border-green-800/30 p-6 md:p-10 flex flex-col">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div class="flex items-center gap-4 bg-white p-4 rounded-3xl border border-green-100 shadow-sm flex-1">
                ...
                <div>
                    ...
                    <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">STATISTIK MUTASI SISWA</h1>
                </div>
            </div>
            ...
            <div class="bg-slate-100 px-5 py-3 rounded-2xl border border-slate-200 self-start md:self-center text-xs flex items-center gap-2">
                ...
                <span class="text-slate-600 font-medium">Statistik Mutasi</span>
            </div>
        </div>
            ...
            <div class="flex flex-wrap items-center gap-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-2">
                        <div class="relative">
                            <select name="tahunawal" id="tahunawal" onChange="change()" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 block w-28 pl-3 pr-8 py-2.5 shadow-sm cursor-pointer">
                                <?	
                                $sql_thn_awal="SELECT DISTINCT YEAR(tglmutasi) as thn_awal FROM mutasisiswa WHERE departemen='$departemen' ORDER BY thn_awal ASC";
                                $result_thn_awal=QueryDb($sql_thn_awal);
                                while($row_thn_awal=mysqli_fetch_array($result_thn_awal)){ ?>
                                    <option value="<?=$row_thn_awal['thn_awal']?>" <?=IntIsSelected($row_thn_awal['thn_awal'],$tahunawal)?>><?=$row_thn_awal['thn_awal']?></option>
                                <? } ?>
                            </select>
                        ...
                        <div class="relative">
                            <select name="tahunakhir" id="tahunakhir" onChange="change()" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 block w-28 pl-3 pr-8 py-2.5 shadow-sm cursor-pointer">
                                <?	
                                $sql_thn_akhir="SELECT DISTINCT YEAR(tglmutasi) as thn_akhir FROM mutasisiswa WHERE departemen='$departemen' ORDER BY thn_akhir DESC";
                                $result_thn_akhir=QueryDb($sql_thn_akhir);
                                while($row_thn_akhir=mysqli_fetch_array($result_thn_akhir)){ ?>
                                    <option value="<?=$row_thn_akhir['thn_akhir']?>" <?=IntIsSelected($row_thn_akhir['thn_akhir'],$tahunakhir)?>><?=$row_thn_akhir['thn_akhir']?></option>
                                <? } ?>
                            </select>
                            <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-slate-500">
                                <i class="fa-solid fa-chevron-down text-[10px]"></i>
        ...
        <!-- Content Area -->
        <div class="flex-1 bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden relative text-center">
            <iframe name="statistik_mutasi_grafik" id="footer_frame" src="blank_statistik.php" class="w-full h-full border-none"></iframe>
        </div>
