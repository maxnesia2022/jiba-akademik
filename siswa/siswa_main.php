...
<head>
    ...
    function change_dep() {
        ...
        document.location.href = "siswa_main.php?departemen="+departemen;
    }
    ...
    function change_tingkat() {
        ...
        document.location.href = "siswa_main.php?tahunajaran="+tahunajaran+"&tingkat="+tingkat+"&departemen="+departemen;
    }
    ...
    function change_kelas() {	
        ...
        document.location.href = "siswa_main.php?tahunajaran="+tahunajaran+"&tingkat="+tingkat+"&departemen="+departemen+"&kelas="+kelas;
    }
    ...
    function show_siswa() {
        ...
        document.getElementById('footer_frame').src = "siswa_content.php?departemen="+departemen+"&tahunajaran="+tahunajaran+"&tingkat="+tingkat+"&kelas="+kelas;
    }
...
<body class="bg-green-950 text-slate-800 min-h-screen p-4 md:p-6 select-none overflow-x-hidden">
    ...
    <div class="w-full h-[calc(100vh-3rem)] bg-slate-50 rounded-[2.5rem] md:rounded-[3rem] shadow-2xl border border-green-800/30 p-6 md:p-10 flex flex-col">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div class="flex items-center gap-4 bg-white p-4 rounded-3xl border border-green-100 shadow-sm flex-1">
                ...
                <div>
                    ...
                    <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">PENDATAAN SISWA</h1>
                </div>
            </div>
            ...
            <div class="bg-slate-100 px-5 py-3 rounded-2xl border border-slate-200 self-start md:self-center text-xs flex items-center gap-2">
                ...
                <span class="text-slate-600 font-medium">Pendataan Siswa</span>
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm mb-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div class="flex flex-wrap items-center gap-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative">
                        <select name="tingkat" id="tingkat" onChange="change_tingkat()" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-24 pl-3 pr-8 py-2.5 shadow-sm transition-colors cursor-pointer">
                            <? 
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative">
                        <select name="kelas" id="kelas" onChange="change_kelas()" class="appearance-none bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-64 pl-3 pr-8 py-2.5 shadow-sm transition-colors cursor-pointer">
                            <? 
                            ...
                            while ($row = @mysqli_fetch_array($result)) {
                                ...
                                <option value="<?=urlencode($row['replid'])?>" <?=IntIsSelected($row['replid'], $kelas)?> >
                                    <?=$row['kelas'].', kap: '.$row['kapasitas'].', isi: '.$row1[0]?>
                                </option>
                            <? } ?>
                        </select>
                        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2.5 text-slate-500">
        ...
        <div class="flex-1 bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden relative">
            <iframe name="footer" id="footer_frame" src="blank_siswa.php" class="w-full h-full border-none"></iframe>
        </div>
